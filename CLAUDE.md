...
- **Loki multi-tenant paths** — application logs use `/api/logs/v1/application/...`, infrastructure logs (k8s events) use `/api/logs/v1/infrastructure/...`. The `events` agent uses infrastructure; the `logs` agent uses application.
